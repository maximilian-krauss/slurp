###
	Sherlock - Module to autodetect and render post content
###

_						= require "lodash"
marked			= require "marked"
request 		= require "request"
ytInspector	= require "./inspectors/youtube"
scInspector	= require "./inspectors/soundcloud"
imInspector = require "./inspectors/image"

_isUrl = (title) ->
	urlRegex =/^(?:(?:https?|ftp):\/\/)(?:\S+(?::\S*)?@)?(?:(?!10(?:\.\d{1,3}){3})(?!127(?:\.\d{1,3}){3})(?!169\.254(?:\.\d{1,3}){2})(?!192\.168(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]+-?)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]+-?)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/[^\s]*)?$/i
	match = urlRegex.exec title
	return if match then true else false

_fetchDocumentTitle = (url, cb) ->
	await request.get url, timeout: 1000, defer err, res, body
	if err
		return cb err

	titleRegEx = /(<\s*title[^>]*>(.+?)<\s*\/\s*title)>/gi
	match = titleRegEx.exec body
	if match and match[2]
		return cb null, match[2]

	return cb new Error "No title found"

_renderMarkdown = (postRequestBody, cb) ->
	marked.setOptions
  	renderer: new marked.Renderer()
  	gfm: true
  	tables: true
  	breaks: false
  	pedantic: false
  	sanitize: true
  	smartLists: true

	await marked postRequestBody.content, defer err, renderedString
	if err
		return cb err

	postRequestBody.renderedContent = renderedString
	if _isUrl postRequestBody.title
		postRequestBody.customPayload = postRequestBody.title

		await _fetchDocumentTitle postRequestBody.title, defer err, documentTitle
		if not err and documentTitle
			postRequestBody.title = documentTitle

		postRequestBody.type = "link"
	else
		postRequestBody.type = "markdown"

	cb null, postRequestBody


module.exports.render = (postRequestBody, cb) ->
	inspectors = [
		new imInspector postRequestBody
		new ytInspector postRequestBody
		new scInspector postRequestBody
	]
	inspector = _(inspectors).find (inspector) ->
		inspector.matches()

	if inspector?
		await inspector.render defer err, post
		return cb err, post


	_renderMarkdown postRequestBody, cb
